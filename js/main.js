...
window.addEventListener('scroll', function() {
  ...
  // Adjust the tolerance to account for mobile browsers (e.g., 150px or customize as needed)
  var tolerance = 150;

  // Checking whether partially visible with some tolerance for mobile
  if(position.top <= (window.innerHeight - tolerance) && position.bottom >= tolerance) {
  ...
  // Additional check for when the element is no longer visible
  if(position.top < window.innerHeight && position.bottom >= 0) {
    // Element is partially visible in the screen (adjust this if needed)
  } else {
    $('.counter-value').each(function() {
      var $this = $(this),
        countTo = 0;
